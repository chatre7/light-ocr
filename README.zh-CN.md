...
> **PR #11 的 Native WebGPU 候选：**Linux x64/Vulkan 与 Windows x64/D3D12 已完成基于 ONNX Runtime 1.24.4 和官方 WebGPU Plugin EP 0.1.0 的自包含、可复现源码实现，包括 provider 注册、Auto fallback、runtime descriptor、npm staging、离线完整性校验与硬件无关测试。Linux/Windows 真机资格验证仍待完成，因此当前不宣称兼容范围或加速数据，也未进入已发布的 `0.2.0` packages。
...
PR #11 同时包含 Linux x64 与 Windows x64 Native WebGPU 源码候选。其 npm payload 仍受两个平台真机资格报告的发布门槛约束；报告完成审阅前，已发布的 `0.2.0` packages 保持不变，并在这两个平台继续仅使用 CPU。
