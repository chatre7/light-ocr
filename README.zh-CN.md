...
`light-ocr` 仍在积极开发。`0.2.0` 已发布确定性的 `tiled-v1` 大图模式，以及 Node.js 适配器中受资源限制的内存 JPEG/PNG 解码；C++ Core 的 raw-pixel 边界保持不变。当前 `0.2.1` 源码候选加入 macOS 可选 Direct Core ML 执行，同时继续以 CPU 为默认 provider。
