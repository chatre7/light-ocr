...
- 四个平台的 build job 把已验签 benchmark 工具打成保留可执行位与动态库布局的归档；同 OS/arch 的全新隔离 runner 再保存三个 locked 2048² fixture 的 Core-vs-Python latency 和 Core diagnostics on/off lifetime peak，避免编译与 Node 测试负载污染受审基线。Node 22/24 diagnostics on/off latency/peak 仍由 package job 采集；汇总 job 拒绝缺失矩阵、错误 identity、relative gate、absolute gate 与 accepted baseline 的 15% regression。
