...
首次实现没有旧 tiled baseline 时，candidate 仍必须通过 absolute memory、per-call timeout、同 native runtime 的 Node/Core latency 和全部 quality gates；Core/Python 跨 binary latency 作为 observation 保留。CI 生成待审 JSON；reviewer 核对 runner identity、result hash、波动、profiler 证据和四平台完整性后，把 baseline 文件作为同一 release PR 的受审变更提交。
...
- [ ] 四平台 warm median/p95 基线已受审提交，Node/Core bootstrap 和后续 `1.15x` regression gate 有真实失败测试。
