...
| Perf-2 宿主加速器 Spike | HC 后立即启动；不等待 N2 | 每个 provider 2–4 周 | 复核已接受的 Apple 路径，并按 HC 排序验证 WebGPU、Windows GPU、OpenVINO、CUDA、QNN 等路径 | 每个 provider 得出接受、缩减或拒绝结论；N2/N4 新 workload 加入后复核 |
...
- [`EngineInfo`](../src/core/engine.cpp) 已经按 detector/recognizer session 报告请求 provider、实际 provider chain、设备、precision、runtime、cache 和 fallback，不再固定只有 `CPUExecutionProvider`。
...
已实现的 Direct Core ML FP16 Apple provider、ANE/GPU/CPU 分工和 M4 Max 资格证据记录在 [Apple Device 加速技术方案](apple-device-acceleration.md)；它已在源码与本地 Gate 层面接受，但尚不代表 npm 已发布，也不把未验证 Mac 宣称为同等性能设备。Windows ML 2.x、DirectML 和 Intel/AMD/NVIDIA/Qualcomm provider 的分层策略仍记录在 Draft [Windows Device 加速技术方案](windows-device-acceleration.md)。Linux WebGPU、CUDA、OpenVINO、MIGraphX 与厂商 NPU 的边界记录在 Draft [Linux Device 加速技术方案](linux-device-acceleration.md)。未通过各自 Gate 的 spike 和上游能力不自动成为发布承诺。
...
在决定 WebGPU、Windows GPU、OpenVINO 或 CUDA 等后续候选谁先做之前，先区分三种不能互相替代的“覆盖率”。Apple provider 已完成首个本地 Gate；后续 HC 排序决定的是其发布/扩证投入和下一条 provider 路线，而不是撤销已获得的证据：
...
| --- | --- | --- | --- | --- |
| ONNX Runtime Native WebGPU EP | 通过 Dawn 在 Windows 使用 D3D12/Vulkan、Linux 使用 Vulkan、macOS 使用 Metal；初始插件提供 Windows x64/arm64、Linux x64 与 macOS arm64 二进制 | Windows x64、Linux x64、macOS arm64 | 初始插件为 `v0.1.0` 且要求兼容的 ORT 1.24.4+；无 macOS x64、Linux arm64 和 NPU；当前动态 shape、CPU↔GPU copy、驱动差异、包体与端到端收益均待项目验证 | 技术横向覆盖广，可作为跨厂商 GPU baseline 候选；不称为全平台 fallback，也不因 API 统一而自动排在厂商 EP 之前 |
...
- 条件分支是：跨厂商 GPU 且可接受初始插件风险的用户权重最高则优先 WebGPU；Windows 混合 GPU 最高则优先 Windows ML/DirectML；Intel 跨 Windows/Linux 最高则优先 OpenVINO；Linux/Windows NVIDIA 服务端 workload 最高则优先 CUDA。Apple 的后续投入按发布与扩证收益排序；QNN、Vitis AI、MIGraphX 只有在对应平台进入目标矩阵且用户样本达到 D111 门槛后前移。
...
`apple` 已进入 `0.2.1` 源码候选 union，但尚未进入已发布的 `0.2.0` packages；其余 accelerator 名称只是 Perf-2 候选。只有已经通过 PG、由 D111 接受且实际随对应 release set 交付的名称，才进入该版本发布的 TypeScript union。Windows ML 在确定离线、固定版本的 bring-your-own EP 路径前不进入稳定枚举。
...
- `cpuPartition` 控制同一个 ORT session 是否允许将未覆盖节点分配给 CPU：项目级兼容默认仍为 `allow`，但每个 provider 的 qualification 必须另跑 `forbid` 以证明完整 coverage；该 provider 首次公开时采用哪个默认值由 D111 在 benchmark 前锁定。允许 partition 时必须披露 placement 并通过同一端到端 PG；它与 `sessionFallback` 是两条不同策略；
